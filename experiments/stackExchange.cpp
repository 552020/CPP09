...
template <typename Iterator>
auto operator+(group_iterator<Iterator> it, std::size_t size) -> group_iterator<Iterator>
{
	return it += size;
}

template <typename Iterator>
auto operator+(std::size_t size, group_iterator<Iterator> it) -> group_iterator<Iterator>
{
	return it += size;
}
...
auto merge_insertion_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
{
	...
	// unneeded)
	static constexpr std::uint_least64_t jacobsthal_diff[] = {2u,
															  2u,
															  6u,
															  10u,
															  22u,
	...
	for (int k = 0;; ++k)
	{
		...
		while (true)
		{
			auto insertion_point = std::upper_bound(
				chain.begin(), it->next, it->it, [=](auto lhs, auto rhs) { return compare(*lhs, *rhs); });
	...
	while (not pend.empty())
	{
		...
		auto insertion_point =
			std::upper_bound(chain.begin(), it->next, it->it, [=](auto lhs, auto rhs) { return compare(*lhs, *rhs); });
		chain.insert(insertion_point, it->it);
		pend.pop_back();
	}
